...
class Router (object):
    ...
    def __init__(self, connection):
        ...
        self.routing_table = {'10.0.1.0/24': ['10.0.1.100', 's1-eth1', '10.0.1.1', 1, '00:00:00:00:00:01'], '10.0.2.0/24': [
            '10.0.2.100', 's1-eth2', '10.0.2.1', 2, '00:00:00:00:00:02'], '10.0.3.0/24': ['10.0.3.100', 's1-eth3', '10.0.3.1', 3, '00:00:00:00:00:03']}
    ...
    def act_like_router(self, packet, packet_in):
        ...
        if packet.type == pkt.ethernet.ARP_TYPE:
            if packet.payload.opcode == pkt.arp.REQUEST:
                ...
                arp_reply = pkt.arp()
                arp_reply.hwsrc = adr.EthAddr("40:10:40:10:40:10")  # some MAC
                ...
                if icmp_packet.type == pkt.TYPE_ECHO_REQUEST:
                    ...
                    for key in self.routing_table.keys():
                        if dst_ip.inNetwork(key):
                            ...
                            break
                    if k:
                        log.debug("ICMP reply sent to " + key)
                        # create echo packet
                        ech = pkt.echo()
            ...
            # Other than ICMP
                ...
                for key in self.routing_table.keys():
                    if dst_ip.inNetwork(key):
                        ...
                        break
                if found_key != 0:
                    port1 = self.routing_table[found_key][3]
                    dsteth = adr.EthAddr(self.routing_table[found_key][4])
    ...
    def FlowMode(self, packet_in, out_port):
        log.debug("Flow mode installed successfully!")
        msg = of.ofp_flow_mod()
